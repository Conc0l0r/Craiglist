...
if (isset($conn)) {
    ...
    if ($check && $check->num_rows > 0) {
        $result = $conn->query("SELECT *, 'Medical' AS category FROM `$table` ORDER BY created_at DESC");
        if ($result) {
...
<main>
    <div class="container">
        <h1 class="page-title">Medical & Healthcare Jobs</h1>
        
        <?php if (empty($jobs)): ?>
            <div style="text-align: center; padding: 3rem;">
                <p style="font-size: 1.2rem; color: #666;">No Medical jobs found. <a href="post.php">Post one now</a>!</p>
            </div>
        <?php else: ?>
            <div class="jobs-grid">
                        ...
                        <?php if (!$isVacant && !$isOwnJob): ?>
                            <div class="restricted-info">
                                ...
                                <div style="margin-top: 0.5rem;">
                                    <a href="ad-detail.php?id=<?php echo (int)$job['id']; ?>&table=<?php echo urlencode($table); ?>" class="btn" style="padding: 0.4rem 0.8rem; font-size: 0.85rem;">
                                        View details
                                    </a>
                                </div>
                            </div>
                        <?php else: ?>
                            <?php if (isLoggedIn()): ?>
                                <div class="full-info">
                                    ...
                                    <div class="info-item">
                                        <a href="ad-detail.php?id=<?php echo (int)$job['id']; ?>&table=<?php echo urlencode($table); ?>" class="btn" style="padding: 0.4rem 0.8rem; font-size: 0.85rem; margin-top: 0.5rem;">
                                            View details
                                        </a>
                                    </div>
                                </div>
