...
$page_title = 'Marketing - Craigslist';

$jobs = [];
$table = 'jobs_marketing';
if (isset($conn)) {
    $check = $conn->query("SHOW TABLES LIKE '$table'");
    if ($check && $check->num_rows > 0) {
        $result = $conn->query("SELECT *, 'Marketing' AS category FROM `$table` ORDER BY created_at DESC");
        if ($result) {
...
<main>
    <div class="container">
        <h1 class="page-title">Marketing Jobs</h1>
        
        <?php if (empty($jobs)): ?>
            <div style="text-align: center; padding: 3rem;">
                <p style="font-size: 1.2rem; color: #666;">No Marketing jobs found. <a href="post.php">Post one now</a>!</p>
            </div>
        <?php else: ?>
            <div class="jobs-grid">
